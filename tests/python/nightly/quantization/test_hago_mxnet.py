...
# evaluate the model
# ----------------
def eval_model(model_name, use_per_channel, hw_name, skip_fp32=True, soundness_check=False):
    ...
    if not skip_fp32:
        ...
        acc = eval_acc(func, val_data, batch_fn, var_name='data', target=target, ctx=ctx, soundness_check=soundness_check)
        print("fp32_accuracy", model_name, acc, sep=',')

    # Quantize
    calib_dataset = get_calibration_dataset(val_data, batch_fn, var_name='data')
    fp32_mod, params = get_model(model_name)
    qconfig = hago.qconfig(use_channel_quantize=use_per_channel, log_file='temp.log')
    hardware = hago.create_hardware(hw_name)
    quantized_func = quantize_hago(fp32_mod, params, calib_dataset, qconfig, hardware=hardware)
    print(quantized_func)
    acc = eval_acc(quantized_func, val_data, batch_fn, var_name='data', target=target, ctx=ctx, soundness_check=soundness_check)
...
if __name__ == '__main__':
    ...
    for model_name in models:
        eval_model(model_name, args.use_per_channel, args.use_uint8, args.skip_fp32, args.soundness_check)
